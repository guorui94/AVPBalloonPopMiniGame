...
struct MemoryGameImmersive: View {
    ...
    var body: some View {
        RealityView { content, attachments in
            ...
            if let deviceAnchor = pose.worldTracking.queryDeviceAnchor(atTimestamp: CACurrentMediaTime()) {
            ...
            if let baseTile = await getBaseTile() {
                ...
                print("❌ Could not find base tile in ImageAnchorScene.")
            }
        } attachments: {
                            ...
                            if imagePair == firstFlippedImage {
                                ...
                                if cardsPairCount == currentGameMode.images.count / 2 {
                                    ...
                                    if let next = nextMode(after: currentGameMode) {
                                        ...
                                        if let nextBase = await getBaseTile() {
                                            await createGameTiles(gameMode: next,
                                                                  baseTile: nextBase,
                                                                  worldAnchor: worldAnchor)
                                        }
                                ...
                                if let first = firstFlippedEntity { shakeEntity(first) }
                                shakeEntity(entity)
                                try? await Task.sleep(for: .milliseconds(400))
    ...
    func getBaseTile() async -> Entity? {
        ...
        if let scene = try? await Entity(named: "ImageAnchorScene",
                                         in: realityKitContentBundle),
           let tile = scene.findEntity(named: "Tile") {
            return tile
        }
        return nil
    }
        ...
        for row in 0..<rows {
            for col in 0..<columns {
                ...
                guard var pairComponent = tileClone.components[PairComponent.self],
                    ...
                    fatalError("Base tile missing PairComponent/ScoreComponent")
                }
                ...
                guard var modelComponent = tileClone.components[ModelComponent.self],
                    ...
                    fatalError("Base tile missing ModelComponent/ShaderGraphMaterial")
                }
